...
only lifecycle/probe capabilities and fixed safe failure; the restricted
`@oms/redis/lua-script` subpath executes only registered static scripts. The
runtime uses one RESP2 client, explicit authentication and TLS policy, bounded
admission and deadlines, no offline queue or automatic in-operation reconnect,
and a terminal bounded shutdown. Script execution tries `EVALSHA`, then exactly
one `EVAL` only for Redis's exact canonical script-cache-miss reply; every
