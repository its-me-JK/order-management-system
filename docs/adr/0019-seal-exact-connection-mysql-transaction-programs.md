...
before Identity can implement its concrete refresh Unit of Work.
...
production runtime and captures the program function once. Program and decoder
functions are invoked receiver-free, so later mutation of their definition
objects cannot change `this`-based behavior. The returned
executor is frozen, has no enumerable resource field, and has one operation:
...
6. seal and drain its statement session;
7. request `COMMIT` or `ROLLBACK` once;
8. terminate the one-use transport and observe its closure/operation barrier.
