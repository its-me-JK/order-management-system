...
| **Total** | **100%** | **39.35%** | Displayed overall is rounded down |
...
The current rotation-mutation rollback increment earns a conservative 0.05
testing/security points and no backend, architecture, platform, documentation,
deployment, or CI/CD points. The guarded disposable database now receives
root-owned, fixture-scoped `AFTER` triggers that make the predecessor-consume,
successor-insert, access-insert, predecessor-link, family-advance, and final
event-append statements fail one at a time. Each target statement leaves no row
effect; each later case cumulatively rolls back all earlier successful writes,
preserves the exact family/credential/event snapshot, and cannot mint credential
delivery. A separately over-bound resulting authority projection fails after
the five graph writes and proves their complete rollback; a clean final rotation
proves the one-slot transaction runtime remains usable. The fixtures require no
production failpoint or additional application privilege. This matrix does not
inject failures into lock acquisition, the post-lock clock read, or the
post-event/pre-`COMMIT` interval, exercise escaped transaction scope, or
simulate protocol-level ambiguous `COMMIT` acknowledgement loss. There is still
no channel-specific credential delivery, public refresh use case, HTTP
