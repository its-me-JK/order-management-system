...
| **Total** | **100%** | **38.20%** | Displayed overall is rounded down |
...
> Overall: 38% · Backend business capabilities: 9.15/35 · Frontend: 0/12 ·
> Deployment: 1.75/8 · Public demo: not deployable

The current direct locked-loader increment earns a conservative 0.10 backend
capability points and 0.10 testing/security points. It earns no architecture,
platform, documentation, deployment, or CI/CD points: one private read adapter
now uses the already-scored executor, but there is still no DML writer,
concrete Unit of Work, safe post-deadline command cleanup, database-gated
completion, credential delivery, HTTP composition, protected route, release
pipeline, or live environment.
...
not-found. The database package owns operation drain, deadline quarantine, and
settlement classification; the future Identity Unit of Work must add writers,
map those outcomes, activate or revoke its exact completion, and safely retire
a running command after a deadline returns `indeterminate`. The
...
direct loader, rotation/reuse writer, concrete Identity Unit of Work,
database-gated completion promotion, credential-delivery gate,
