...
| Testing, security, and resilience | 11% | 7.05% | Strict quality gates, secret-safe configuration/TLS and adversarial HTTP tests, Catalog lifecycle/Unicode tests, exhaustive Identity Account/authenticator/Role tests, adversarial session chronology/generation/expiry/replay tests, authenticated-principal tests, 137 opaque-credential/Node-adapter tests, hardened attempt/discovery/event tests, and 45 workflow tests covering exact wire-to-digest correlation, one-shot ownership, cleanup, ticket forgery, cross-boundary consumption, locked-row relationships, terminal-action provenance, event validation, rotation-only digest access, principal binding, pending-evidence consumption/revocation, hostile re-entrancy, exact result binding, contaminated-error recreation, fixed errors, and package isolation; executable architecture boundaries, real-MySQL Catalog lifecycle/contract suites, the dedicated Identity lineage suite, and an isolated authorization suite proving exact byte-level seeds, idempotent deployment, Prisma drift absence, UUID versions, lifecycle/event compatibility, typed context, microseconds, indexes, mapping uniqueness, `RESTRICT` references, and evidence-retention independence |
...
| CI/CD and public deployment | 8% | 1.75% | CI replays migrations idempotently and validates database, Identity-lineage, Identity-authorization, Catalog, and API contracts against real MySQL; no release pipeline or live environment |
...
| **Total** | **100%** | **35.70%** | Displayed overall is rounded down |
...
The current increment earns no backend-capability or deployment points.
...
before a real use case exists. The ordered Prisma history now contains unused
Identity persistence for Account, SessionFamily, retained refresh generations,
generation-bound access, the exact permission registry and system role, Role
permission mappings, Account role assignments, and typed security evidence.
...
None of these records earns backend-capability or deployment credit because no
production path consumes them. There is still no locked refresh store adapter,
concrete Unit of Work, committed completion, resolver/login/refresh use case,
password-authenticator or bootstrap-state persistence,
authority/security-event adapter, Argon2 provider, password-input policy,
