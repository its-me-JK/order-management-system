...
| **Total** | **100%** | **38.50%** | Displayed overall is rounded down |
...
> Overall: 38% · Backend business capabilities: 9.30/35 · Frontend: 0/12 ·
> Deployment: 1.75/8 · Public demo: not deployable

The current rotation-authority prerequisite earns a conservative 0.05 backend
capability points and 0.05 testing/security points. It earns no architecture,
platform, documentation, deployment, or CI/CD points: one private non-locking
statement can now project current authority after a transaction-local family
update, while the shared mapper keeps Bearer and refresh permission semantics
aligned. There is still no rotation writer, concrete Unit of Work, safe
post-deadline command cleanup, database-gated completion, credential delivery,
...
identifier collides. The database package owns operation drain, deadline
quarantine, and
settlement classification; the future Identity Unit of Work must add the
rotation writer, compose the delivered reuse writer, map those outcomes,
...
resolver is its only public application consumer. There is still no rotation
writer, concrete Identity Unit of Work, database-gated completion promotion,
