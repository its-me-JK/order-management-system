...
| Documentation and demo polish | 5% | 2.85% | README, architecture contracts including the exact authorization registry, security-evidence matrix, connection-budget rules, and commit-ambiguity boundary, ADR history, deterministic OpenAPI JSON, and a public read-only local Swagger UI exist; examples and demo guides remain |
| **Total** | **100%** | **37.35%** | Displayed overall is rounded down |
...
The current direct-connection allocator increment earns the remaining 0.05
platform point, a conservative 0.15 testing/security points, and 0.05
documentation points. It earns no backend-capability, deployment, or CI/CD
points: it adds no transaction-scoped store, DML writer, `BEGIN`,
transaction-level drain, commit settlement, HTTP composition, protected route,
release pipeline, or live environment. The preceding closed refresh command
and Prisma locked-loader increments remain required evidence but are not
recounted.
...
transaction sequencing and drain, commit-settlement classification, committed
