...
| Documentation and demo polish | 5% | 2.95% | README, architecture contracts including the exact authorization registry, security-evidence matrix, connection-budget rules, concrete refresh Unit-of-Work settlement lifecycle, package-internal exact-pair delivery gate, and the sealed prepared-statement/commit-ambiguity boundary, nineteen ADRs, deterministic OpenAPI JSON, and a public read-only local Swagger UI exist; examples and demo guides remain |
| **Total** | **100%** | **39.25%** | Displayed overall is rounded down |
...
> Overall: 39% · Backend business capabilities: 9.75/35 · Frontend: 0/12 ·
> Deployment: 1.75/8 · Public demo: not deployable

The current established-driver deadline increment earns a conservative 0.05
testing/security points and no backend, architecture, platform, documentation,
deployment, or CI/CD points. The guarded real-MySQL runner now holds the exact
Account row before invoking the production refresh Unit of Work on a runtime
with one reserved direct connection. The configured deadline returns the
exact `indeterminate` singleton after a bounded same-user process-list poll has
positively observed the active production Account-lock query. No completion or
delivery authority is available, the original persistence graph remains
unchanged before and after lock release, and a separate rotation commits through
recovered execution capacity. This composes the live deadline/quarantine path
with the application boundary and proves its external guarantees; focused
Unit-of-Work tests separately prove private late close and revocation. It does
not claim physical-session identity, transaction cancellation, protocol-level
commit-acknowledgement loss, or per-operation rollback. There is still no
channel-specific credential delivery, public refresh use case, HTTP composition,
protected route, release pipeline, live environment, or bounded
fail-stop-quarantine observability and process-recycle policy.
...
asynchronous verification. Committed rejection or reuse, foreign or equivalent
pairs, structural values, Proxies, and replay fail without returning delivery
authority. The capability has no raw serializer or public package subpath; the
future access-response and refresh-cookie sinks remain undelivered. The
