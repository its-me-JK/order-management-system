...
| **Total** | **100%** | **39.30%** | Displayed overall is rounded down |
...
The current post-lock-clock and competing-refresh increment earns a conservative
0.05 testing/security points and no backend, architecture, platform,
documentation, deployment, or CI/CD points. A generic transaction-start clock
could become stale while waiting for Account ownership, so the executor now
attests only UTC and `READ-COMMITTED`; Identity's closed 13-statement program
reads one `UTC_TIMESTAMP(6)` only after its last awaited lock. The guarded
real-MySQL runner starts two independently admitted commands from the same
predecessor behind an external Account blocker and positively observes both
production Account-lock queries. After release, exactly one command commits a
rotation and the other commits reuse-family closure. Only the winner generation
persists, the family ends revoked, and only the exact winner completion and
candidate pair can mint delivery authority. It does not claim response order,
general deadlock freedom, physical-session identity, protocol-level
