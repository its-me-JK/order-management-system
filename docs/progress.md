...
| Backend business capabilities | 35% | 7.25% | Catalog public reads and Product/SKU aggregates exist. Identity now has separate immutable Account, PasswordAuthenticator, Role, and SessionFamily boundaries plus versionless RefreshCredential and AccessCredential children. Creation and rotation return complete generation-matched issuance bundles; application code now owns canonical redacting access/refresh wire values, copied-byte digest values, one exact frozen candidate pair, the narrow type-only cryptography port, and the nominal authenticated-principal contract. Identity still has no concrete crypto adapter, resolver/login/refresh use case, or route |
| Redis, RabbitMQ, and workers | 9% | 0% | Architecture only |
| Testing, security, and resilience | 11% | 6.05% | Strict quality gates, secret-safe configuration/TLS and adversarial HTTP tests, Catalog lifecycle/Unicode tests, exhaustive Identity Account/authenticator/Role tests, adversarial session chronology/generation/expiry/replay tests, authenticated-principal tests, and 103 opaque-credential tests covering every canonical Base64url tail, namespace separation, redaction, copied-byte aliasing, detached/shared/resizable storage, prototype forgery, exact candidate shape, and entropy-reuse rejection; executable architecture boundaries, real-MySQL migration suites, and a real NestJS-to-Prisma-to-MySQL Catalog contract suite |
...
| **Total** | **100%** | **33.55%** | Displayed overall is rounded down |
...
> Overall: 33% · Backend business capabilities: 7.25/35 · Frontend: 0/12 ·
...
type-only `IdentityAuthenticatedPrincipal` contract. There is still no
concrete cryptographic adapter, resolver/login/refresh use case, Unit of Work,
Identity MySQL schema, Argon2 provider, password-input policy, offline command,
