...
| Backend business capabilities | 35% | 3.25% | Catalog Product/SKU persistence, UUIDv7 binary mapping, active-only reads, exact seek pagination, bounded public-read use cases, exact anonymous endpoints, and an immutable Product aggregate with explicit reversible lifecycle, validated rehydration, versions, timestamps, and internal events exist; no Catalog write use case exists |
| Redis, RabbitMQ, and workers | 9% | 0% | Architecture only |
| Testing, security, and resilience | 11% | 4.25% | Strict quality gates, secret-safe configuration and TLS tests, adversarial HTTP, cursor, Product lifecycle/Unicode, and OpenAPI tests, executable Clean Architecture boundaries, isolated real-MySQL migration and constraint tests, and a real NestJS-to-Prisma-to-MySQL Catalog contract suite |
...
| **Total** | **100%** | **26.40%** | Displayed overall is rounded down |
...
> Overall: 26% · Backend business capabilities: 3.25/35 · Frontend: 0/12 ·
...
Product aggregate earns domain-behavior credit but remains unwired until the
forward lifecycle migration and later application/Unit-of-Work slices exist.
