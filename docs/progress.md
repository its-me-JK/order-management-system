...
| CI/CD and public deployment | 8% | 1.80% | CI replays migrations idempotently and validates database, Identity-lineage, Identity-authorization, Identity-authority runtime, Identity refresh-discovery, Identity refresh transaction composition, Catalog, and API contracts against real MySQL, plus the bounded runtime against authenticated ephemeral real Redis; no release pipeline or live environment |
...
| **Total** | **100%** | **40.05%** | Displayed overall is rounded down |
...
> Overall: 40% · Backend business capabilities: 9.95/35 · Frontend: 0/12 ·
> Deployment: 1.80/8 · Public demo: not deployable

The current technical Redis increment earns a conservative 0.20 Redis/worker,
0.05 testing/security, and 0.05 CI/deployment points, with no backend business,
frontend, observability, architecture, platform/persistence, or public-demo
credit. `@oms/redis` is now the only owner of exact `@redis/client@6.2.1` and
provides one process runtime with explicit RESP2, authentication/TLS policy,
bounded queue and deadlines, disabled offline queue and automatic
in-operation reconnect, fixed safe failures, and bounded terminal shutdown.
Its package root grants lifecycle/probe only. The restricted registered-script
surface validates exact key/argument counts, uses `EVALSHA`, permits exactly one
`EVAL` only for Redis's exact canonical script-cache-miss reply under the
reviewed-script invariant, and fails every ambiguous outcome closed without
retry. Local Compose and CI now supply authenticated ephemeral Redis
7.2.16 pinned by digest, a least-privilege `oms_app` user, persistence disabled,
64 MiB `noeviction`, and loopback publication. Focused package tests and a
real-Redis gate prove the technical boundary. This does not implement the
Identity script, key HMAC, abuse-control adapter, trusted proxy provenance,
actual admission decision, API/worker composition, route, caching, global
readiness, release pipeline, or live environment.
...
abuse-control adapter and runtime composition,
